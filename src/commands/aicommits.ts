...
export default async (generate: number, rawArgv: string[]) =>
	(async () => {
		...
		const messages = await generateCommitMessage(OPENAI_KEY, staged.diff, generate);
		...
		await execa('git', ['commit', '-m', `${commitType}: ${message.toLowerCase()}`, ...rawArgv]);
